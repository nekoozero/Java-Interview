...
java中的**队列**的接口是 Queue，**栈**的接口是 Deque（继承了Queue，stack接口过时了），LinkedList实现了Deque接口。

Deque本质上是一个双端队列（栈操作建议使用，限制双端队列在一端进行插入和删除就成了栈）。

- java中实现栈和队列操作都可以使用LinkedList实现，当然底层使用的是链表。